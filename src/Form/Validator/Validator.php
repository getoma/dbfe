...
class Validator
{
   /** stores validity status for each field */
   public array $is_valid  = [];
   /** the unprocessed input data as received */
   public array $data     = [];
   /** the processed/filtered data after validation */
   public array $filtered = [];
   /** @var string any global status message to display to the user */
   public $form_msg = '';
   /** @var string[]] any field specific error messages to display to the user */
   public array $msg      = [];
   ...
   {
      if( is_array($profile) ) $profile = new Profile($profile);
      $this->profile = new Configuration($profile);
   }
    ...
    * @return string
    */
   protected function checkField( Field $profile, $data)
   {
      $errmsg = '';
    ...
    */
   protected function filterField( Field $profile, $data)
   {
      /*
